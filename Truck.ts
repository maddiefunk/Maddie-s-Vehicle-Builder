...
class Truck extends Vehicle implements AbleToTow {
  ...
  // implements tow method from AbleToTow interface
  tow(vehicle: Truck | Motorbike | Car): void {
    if (!vehicle.make || !vehicle.model) {
      console.log("The vehicle doesn't have a valid make and model.");
      return;
    }
    //checks if vehicle's weight is > or = to trucks towing capacity 
    if (vehicle.weight <= this.towingCapacity) {
      console.log(`Towing vehicle: ${vehicle.make} ${vehicle.model}`);
    } else {
      console.log(`This vehicle is too heavy to be towed. Weight: ${vehicle.weight}, Towing Capacity: ${this.towingCapacity}`);
    }
  }
